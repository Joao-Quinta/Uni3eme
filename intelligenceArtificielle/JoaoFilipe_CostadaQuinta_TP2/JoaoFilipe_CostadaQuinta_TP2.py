...
def algoRechercheGeneral():
    ...
    # on initialise la liste d etats à explorer, on a deux sous listes, la première les eétats, la deuxième les états
    # parents des états à explorer etatAVisiter[0][i] = etat, etatAVisiter[1][i] = le parent de l'état 
    etatAVisister[0].append(etatInitial.copy())
    etatAVisister[1].append([])
    while len(etatAVisister[0]) > 0:
        ...
        # si c'est pas final, alors on marque l'état courant comme déjà visité, pour pas creer des cycles dans notre 
        # exploration on le fait en creeant la liste des etats deja explorés, qui a deux sous listes, la premiere 
        # l'état, et dans la deuxième le parent correspondant etatDejaVisite[0][i] = etat, etatDejaVisite[1][i] = le 
        # parent de l'état 
        etatDejaVisite[0].append(etatCourant.copy())
        etatDejaVisite[1].append(parentEtatCourant.copy())
        ...
        # pour chaque transition possible on calcule l'état resultant suite à l'application de la transition à notre 
        # état courant 
        for i in range(0, len(listePossibilites)):
            listeEtatCalcules.append(changementEtat(copy.deepcopy(etatCourant), listePossibilites[i]))

        # pour tout état qu'on peut "trouver" depuis état conrant, on vérifie qu'on l'a pas encore explorer avc un 
        # simple if (on évite ainsi les cycles) s il est non visité, alors on le met dans la liste des états qu'on a 
        # a explrer, et on stock son parent aussi qui est en fait l'état courant 
        for i in range(0, len(listeEtatCalcules)):
